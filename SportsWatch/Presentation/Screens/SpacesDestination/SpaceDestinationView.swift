...
extension Entity {
    func addSkybox(for destination: SpaceDestination) {
        let subscription = TextureResource.loadAsync(named: destination.url).sink(
            receiveCompletion: {
                switch $0 {
                case .finished: break
...
//                    mesh: .generateSphere(radius: 1E3),
    ...
    func updateTexture(for destination: SpaceDestination) {
        let subscription = TextureResource.loadAsync(named: destination.url).sink(
            receiveCompletion: {
                switch $0 {
                case .finished: break
